            ...
            <div class="container-fh__content dataset">
                <div class="dataset__header">
                    <div class="dataset__header-side">
                        <div class="dataset__header-heading">List Shop For Free </div>
                    ...
                    <table id="demo-foo-addrow"
                        ...
                        <tbody>
                            ...
                            <tr v-show="!rewardpoints.length">
                                <td v-if="$can('admin')" colspan="8" class="no-data-found-info">No shop for free gifts
                                    found</td>
                                <td v-if="!$can('admin')" colspan="7" class="no-data-found-info">No shop for free gifts
                                    found</td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="col">
                    <paginate v-if="totalPages > 1" :page-count="totalPages" :page-range="3" :margin-pages="2"
                        :click-handler="searchGifts(1)" :container-class="'pagination float-right'"
        ...
        methods: {
            searchGifts: function (page) {
               this.filter.customer_id = this.$route.params.customer_id;
                axios.get('/customer-free-gift/reports', {params : this.filter})
                    .then(r => r.data)
                    .then((response) => {
            ...
            edit: function () {
                axios.post('/reward-point/edit/' + this.gift.id, {
                        'gift_code': this.gift.code
                    })
                    .then(r => r.data)
                    .then((response) => {
                        this.errors = null;
            ...
            redeemedGift: function (id, index) {
                axios.get('/gift/redeemed/' + id)
                    .then(r => r.data)
                    .then((response) => {
                        this.searchGifts(this.currentPage);
                    });
            }
        }
    }
