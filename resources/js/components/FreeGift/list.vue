                ...
                <div>
                    ...
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                             <router-link :to="{ name:'FreeGiftCampaigns'}"
                                         tag="a">Shop for free Campaigns</router-link>
                        </li>
                    ...
                    <table id="demo-foo-addrow"
                        ...
                        <tbody>
                            <tr v-for="(gift , index) in gifts" :key="gift.id">
                                ...
                                <td>{{ gift.name }}</td>
                                <td>{{ gift.code }}</td>
                                <td>{{ gift.no_of_gifts }} </td>
                                <td>{{ gift.used_gifts }}</td>
                                <!-- <td>{{ gift.start_at }} </td>
                                <td>{{ gift.end_at }} </td> -->
                                <td>{{ gift.created_at }} </td>
                                <td>
                                     <router-link tag="button"
                                    :to="{ name:'FreeGiftItems', params: { 'gift_id': gift.id  }}" 
                                    class="btn btn-sm btn-icon btn-success" type="button"><span class="fa fa-gift"></span></router-link> 
                                    <router-link tag="button"
                                    :to="{ name:'editFreeGift', params: { 'gift_id': gift.id , 'campaign_id': $route.params.campaign_id }}" 
                                    class="btn btn-sm btn-icon btn-info" type="button"><span class="fa fa-edit"></span></router-link> 
                                    <button @click="remove(gift.id , index)" type="button" class="btn btn-sm btn-icon btn-danger"><span class="fa fa-trash"></span></button>
                                </td>                               
...
<script>
    ...
    export default {
        ...
        methods: {
            reports: function () {
                axios.get('/campaign/free-gift/'+ this.$route.params.campaign_id +'/report')
                    .then(r => r.data)
                    .then((response) => {
            ...
            searchGifts: function (page) {
                ...
                axios.post('/free-gift/search/' + page, this.filter)
                    .then(r => r.data)
                    .then((response) => {
                        this.gifts = response.data;
                        this.totalItems = response.totalItems;
            ...
            remove: function (id, index) {
                this.$swal({
                    ...
                    if (result.value) { // <-- if confirmed
                        axios.delete('/free-gift/remove/' + id)
                            .then(r => r.data)
                            .then((response) => {
            ...
            giftWinners: function (id) {
                this.$router.push({
                    path: '/free-gifts/' + id + '/winners'
                });
            },
            importGifts: function () {
                this.$router.push({
                    name: 'importFreeGifts' , params: { 'campaign_id' : this.$route.params.campaign_id }
                });
            },
            ...
            createGift: function () {
                console.log("sdfdfsfssdf");
                
                this.$router.push({
                    name: 'createGiftFree' , params: { 'campaign_id' : this.$route.params.campaign_id }
                });
            }
        }
    }
