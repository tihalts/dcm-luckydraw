    ...
    <div class="page-content">
        <div class="container-fluid container-fh">
            <div class="page-content__header">
                <div>
                    <h2 class="page-content__header-heading">Country Wise Customers</h2>
                </div>
            ...
            <div class="container-fh__content dataset">
                <div class="dataset__header">
                    <div class="dataset__header-side">
                        <div class="dataset__header-heading">Country Wise Customers</div>
                    ...
                    <div class="dataset__header-controls">
                        <div class="input-group input-group-icon icon-right dataset__header-search">
                            <input class="form-control dataset__header-search-input" v-model="filter.searchText"
                                v-on:keyup="searchCountries(1)" type="text" placeholder="Search">
                    ...
                    <table id="demo-foo-addrow"
                        ...
                        <tbody>
                            <tr v-for="( country , index ) in countries" :key="country.id">
                                ...
                                <td>{{ country.name}} </td>
                                <td>{{ country.iso }} </td>
                                <td>{{ country.total_users }} </td>
                            </tr>
                            <tr v-show="!countries.length">
                                <td colspan="4" class="no-data-found-info">No Countries found</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col">
                    <paginate v-if="totalPages > 1" :page-count="totalPages" :page-range="3" :margin-pages="2"
                        :click-handler="searchCountries" :container-class="'pagination float-right'"
    ...
    export default {
        ...
        methods: {
            getCountries: function () {
                axios.get('/reports/customer/by-country/list' , { params:  this.filter })
                    .then(r => r.data)
                    .then((response) => {
            ...
            searchCountries: function (page) {
                this.filter.page = page;
                var config = {
                    'params': this.filter
                };

                axios.get('/reports/customer/by-country/list', config)
                    .then(r => r.data)
                    .then((response) => {
                        this.countries = response.data;
                        this.totalItems = response.totalItems;
            ...
            exportPdf: function(type){
                ...
                let routeData = this.$router.resolve({
                    path: '/reports/customer/by-country/' + type,
                    query: this.filter
                });
                window.open(routeData.href, '_blank');
            },
