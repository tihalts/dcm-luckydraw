    ...
    <div class="page-content">
        <div class="container-fluid container-fh">
            <div class="page-content__header">
                <div>
                    <h2 class="page-content__header-heading">Promoter Wise Vouchers</h2>
                </div>
            ...
            <div class="container-fh__content dataset">
                <div class="dataset__header">
                    <div class="dataset__header-side">
                        <div class="dataset__header-heading">Promoter Wise Vouchers</div>
                    ...
                    <div class="dataset__header-controls">
                        <div class="input-group input-group-icon icon-right dataset__header-search">
                            <input class="form-control dataset__header-search-input" v-model="filter.searchText"
                                v-on:keyup="searchPromoters(1)" type="text" placeholder="Search">
                    ...
                    <table id="demo-foo-addrow"
                        ...
                        <tbody>
                            <tr v-for="( promoter , index ) in promoters" :key="promoter.id">
                                ...
                                <td>{{ promoter.total_vouchers }} </td>
                            </tr>
                            <tr v-show="!promoters.length">
                                <td colspan="6" class="no-data-found-info">No Promoters found</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div class="col">
                    <paginate v-if="totalPages > 1" :page-count="totalPages" :page-range="3" :margin-pages="2"
                        :click-handler="searchPromoters" :container-class="'pagination float-right'"
    ...
    export default {
        ...
        methods: {
            getPromoters: function () {
                axios.get('/reports/voucher/by-promoter/list' , { params:  this.filter })
                    .then(r => r.data)
                    .then((response) => {
            ...
            searchPromoters: function (page) {
                this.filter.page = page;
                var config = {
                    'params': this.filter
                };

                axios.get('/reports/voucher/by-promoter/list', config)
                    .then(r => r.data)
                    .then((response) => {
                        this.promoters = response.data;
                        this.totalItems = response.totalItems;
            ...
            exportPdf: function(type){
                ...
                let routeData = this.$router.resolve({
                    path: '/reports/voucher/by-promoter/' + type,
                    query: this.filter
                });
                window.open(routeData.href, '_blank');
            },
