    ...
    <div class="page-content">
        <div class="container-fluid container-fh">
            <div class="page-content__header">
                <div>
                    <h2 class="page-content__header-heading">Campaign Wise Gifts</h2>
                </div>
            ...
            <div class="container-fh__content dataset">
                <div class="dataset__header">
                    <div class="dataset__header-side">
                        <div class="dataset__header-heading">Campaign Wise Gifts</div>
                    ...
                    <table id="demo-foo-addrow"
                        ...
                        <tbody>
                            <tr v-for="( campaign , index ) in campaigns" :key="campaign.id">
                                ...
                                <td>{{ campaign.gifts }}</td>
                                <td>{{ campaign.ungifts }}</td>
                                <td>{{ campaign.gifted }} </td>
                            </tr>
                            <tr v-show="!campaigns.length">
                                <td colspan="5" class="no-data-found-info">No Campaigns found</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    export default {
        ...
        methods: {
            getCampaigns: function () {
                axios.get('/reports/gift/by-campaign/list' , { params:  this.filter })
                    .then(r => r.data)
                    .then((response) => {
            ...
            searchCampaigns: function (page) {
                ...
                axios.get('/reports/gift/by-campaign/list', config)
                    .then(r => r.data)
                    .then((response) => {
                        this.campaigns = response.data;
                        this.totalItems = response.totalItems;
            ...
            exportPdf: function(type){
                ...
                let routeData = this.$router.resolve({
                    path: '/reports/gift/by-campaign/' + type,
                    query: this.filter
                });
                window.open(routeData.href, '_blank');
            },
