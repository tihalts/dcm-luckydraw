    ...
    <div class="page-content">
        <div class="container-fluid container-fh">
            <div class="page-content__header">
                <div>
                    <h2 class="page-content__header-heading">Spin Winners</h2>
                </div>
            </div>
            <div class="container-fh__content dataset">
                <div class="dataset__header">
                    <div class="dataset__header-side">
                        <div class="dataset__header-heading">List Spin Winners</div>
                    ...
                    <div class="dataset__header-controls">
                        <div class="input-group input-group-icon icon-right dataset__header-search">
                            <input class="form-control dataset__header-search-input" v-model="filter.searchText"
                                v-on:keyup="searchSpinWinners(1)" type="text" placeholder="Search">
                    ...
                    <div style="container">
                        <div class="from-block">
                            ...
                            <div class="form-group text-right m-b-0">
                                ...
                                <button type="button" @click="searchSpinWinners(1)"
                    ...
                    <table id="demo-foo-addrow"
                        ...
                        <tbody>
                            <tr v-for="( spinner , index ) in spinners" :key="spinner.id">
                                ...
                                <td>
                                    <div class="table__cell-widget" v-if="spinner.customer">
                                        <router-link href="javascript:void(0)" :to="{ name:'CustomerReportPurchases', params: { 'customer_id': spinner.customer.id }}" tag="a" class="table__cell-widget-name">{{ spinner.customer.fullname }}</router-link>
                                        <span class="table__cell-widget-description"><b>Email :</b> {{ spinner.customer.email }}</span>
                                        <span class="table__cell-widget-description"><b>CPR :</b> {{ spinner.customer.cpr }}</span>
                                        <span class="table__cell-widget-description"><b>Mobile :</b> {{ spinner.customer.mobile }}</span>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="table__cell-widget" v-if="spinner.provider">
                                        <router-link href="javascript:void(0)" :to="{ name:'PromoterReportPurchases', params: { 'promoter_id': spinner.provider.id }}" tag="a" class="table__cell-widget-name">{{ spinner.provider.fullname }}</router-link>
                            ...
                            <tr v-show="!spinners.length">
                                <td v-if="user.role == 'admin'" colspan="7" class="no-data-found-info">No Spin Winners found</td>
                                <td v-if="user.role != 'admin'" colspan="6" class="no-data-found-info">No Spin Winners found</td>
                            </tr>
                ...
                <div class="col">
                    <paginate v-if="totalPages > 1" :page-count="totalPages" :page-range="3" :margin-pages="2"
                        :click-handler="searchSpinWinners" :container-class="'pagination float-right'"
...
<script>
    ...
    export default {
        ...
        methods: {
            getSpinWinners: function () {
                axios.get('/spinner/winner/list')
                    .then(r => r.data)
                    .then((response) => {
            ...
            searchSpinWinners: function (page) {
                console.log(this.filter);

                axios.post('/spinner/winner/search/' + page, this.filter)
                    .then(r => r.data)
                    .then((response) => {
                        this.spinners = response.data;
                        this.totalItems = response.totalItems;
