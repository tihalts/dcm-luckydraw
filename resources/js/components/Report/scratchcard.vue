                                ...
                                <div style="padding:20px;">
                                    <div class="from-block">
                                        ...
                                        <div class="form-group text-right m-b-0">
                                            ...
                                            <button type="button" @click="searchScratchCards(1)" class="btn btn-success mb-2 mr-3">Search</button>
                                        </div>
                                        <!-- /.box-footer -->
                                    </div>
                                </div>
                    ...
                    <table id="demo-foo-addrow"
                        ...
                        <tbody>
                            <tr v-for="( scratchcard , index ) in scratchcards" :key="scratchcard.id">
                                ...
                                <td>
                                    <div class="table__cell-widget" v-if="scratchcard.customer">
                                        <a href="javascript:void(0)" class="table__cell-widget-name">{{ scratchcard.customer.fullname }}</a>
                                ...
                                <td>
                                    <div class="table__cell-widget" v-if="scratchcard.provider">
                                        <a href="javascript:void(0)" class="table__cell-widget-name">{{ scratchcard.provider.fullname }}</a>
                            ...
                            <tr v-show="!scratchcards.length">
                                <td colspan="7" class="no-data-found-info">No Scratch Cards found</td>
                            </tr>
    ...
    export default {
        data() {
            return {
                ...
                chartDataHeader: ['Date', 'Scratch Cards'],
                updatedChartData: [],
                chartOptions: {
                    chart: {
                        title: 'Scratch Card Report',
                        subtitle: 'Scratch Cards'
                    },
                    bars: 'vertical',
                },
            }
        ...
        methods: {
            getCharts: function (config = {}) {
                axios.get('/fetch/scratch-card/linechart' , config)
                    .then(r => r.data)
                    .then((response) => {
            ...
            getScratchCards: function () {
                axios.get('/scratch-card/reports')
                    .then(r => r.data)
                    .then((response) => {
            ...
            searchScratchCards: function (page) {
                ...
                axios.get('/scratch-card/reports', config)
                    .then(r => r.data)
                    .then((response) => {
                        this.scratchcards = response.data;
                        this.totalItems = response.totalItems;
            ...
            exportPdf: function(type){
                ...
                let routeData = this.$router.resolve({
                    path: '/scratch-card/' + type + '/exports',
                    query: this.filter
                });
                window.open(routeData.href, '_blank');
            },
