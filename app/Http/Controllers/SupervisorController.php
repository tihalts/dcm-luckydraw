...
class SupervisorController extends Controller 
{
    ...
    {     
        try {        
            $supervisors = App\User::where('role' , 'supervisor');   
            $this->response["totalItems"] = $supervisors->count(); 
            $this->response['data'] = $supervisors->latest()
    ...
    public function search(Request $request , $page = 1)
    {
        try {      
            $filter = $request->all(); 
            $user = App\User::where('role' , 'supervisor');
    ...
    public function update(Request $request , $id)
    {
        ...
        try {
            ...
            if ($validator->fails()) {
                ...
                $user->email = $request->input('email');
                $user->mobile = !Str::contains($request->input('mobile') , $request->input('dialCode')) && $request->input('dialCode') ? $request->input('dialCode'). $request->input('mobile') : $request->input('mobile') ;
                $user->role = $request->has('role') ? $request->input('role') : 'supervisor';
                $user->country_iso = $request->input('country_iso');
                $user->phone_code = $country->phone_code;
                $user->save();
    ...
    public function destroy($id)
    {
        try{
            $user = App\User::find($id);
            if(!isset($user)){
                $this->response["message"] = 'Supervisor not found.!';
            }
            $user->update(['status' => false]);
            $this->response["message"] = 'Supervisor deactivated successfull.!';
            $this->response["data"] = $user;
            return $this->successResponse($this->response);
        } catch (\Exception $e) {
            $this->response['error_message'] = $e->getMessage();
    ...
    public function Activated($id)
    {
        try{
            $user = App\User::find($id);
            if(!isset($user)){
                $this->response["message"] = 'Supervisor User not found.!';
            }
            $user->update(['status' => true]);
            $this->response["message"] = 'Supervisor User activated.!';
            $this->response["data"] = $user;
            return $this->successResponse($this->response);
        } catch (\Exception $e) {
            $this->response['error_message'] = $e->getMessage();
